    ...
    <div class="row mt-4">
        @foreach ($templates as $key => $template)
                    ...
                    <span class="template-image-box2">
                        ...
                        <div class="preview_control">
                            <div style="width: calc(80% - 0px);">
                                <div class="mb-2">
                                    <a class="btn btn-light d-block" href="#preview" onclick="popupwindow('{{ action('TemplateController@preview', $template->uid) }}', `{{ $template->name }}`, 800)"><span class="material-symbols-rounded">
                                        zoom_in
                                    ...
                                    <div>
                                        @if (Auth::user()->customer->can('update', $template))
                                            @if (in_array(Acelle\Model\Setting::get('builder'), ['both','pro']) && $template->builder)
                                                <div>
                                                    <a href="{{ action('TemplateController@builderEdit', $template->uid) }}" role="button" class="btn btn-primary btn-icon template-compose d-block mb-2">
                                            ...
                                            @if (in_array(Acelle\Model\Setting::get('builder'), ['both','classic']))
                                                <div>
                                                    <a href="{{ action('TemplateController@edit', $template->uid) }}" role="button" class="btn btn-info btn-icon template-compose-classic d-block">
                    ...
                    <div class="card-body p-3">
                        ...
                        overflow: hidden;" class="card-text">
                            @if ($template->categories()->count())
                                <span class="template-categories">{{ $template->categories->map(function ($cat) {
                                    return $cat->name;
                                })->join(', ') }}</span>
                            @else
                                <span style="" class="xtooltip" title="{{ trans('messages.template.category.notice') }}">{{ trans('messages.template.category.no_category') }}</span>
                            @endif
                        </p>
                        <div class="">
                                ...
                                @if (Auth::user()->customer->can('preview', $template) ||
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            @endif
                                            @if (Auth::user()->customer->can('preview', $template))
                                                <li><a class="dropdown-item" href="#preview" onclick="popupwindow('{{ action('TemplateController@preview', $template->uid) }}', `{{ $template->name }}`, 800)"><span class="material-symbols-rounded">
            zoom_in
            </span> {{ trans("messages.preview") }}</a></li>
                                            @endif
                                            @if (Auth::user()->customer->can('update', $template))
                                                <li>
                                                    <a class="dropdown-item upload-thumb-button" href="{{ action('TemplateController@updateThumb', $template->uid) }}">
            ...
            insert_photo
                                            ...
                                            @if (Auth::user()->customer->can('update', $template))
                                                <li>
                                                    <a class="dropdown-item template-categories" href="{{ action('TemplateController@categories', [
                                                        'uid' => $template->uid,
                                                    ]) }}">
                                                        <span class="material-symbols-rounded">
            category
                                            ...
                                            @if (Auth::user()->customer->can('read', $template))
                                                <li>
                                                    <a
                                                        href="{{ action('TemplateController@export', $template->uid) }}"
                                                        role="button"
                                                        class="dropdown-item"
                                                        link-method="POST"
                                                    >
