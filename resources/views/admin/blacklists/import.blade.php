...
@section('page_header')
    ...
    <div class="page-title">
        <ul class="breadcrumb breadcrumb-caret position-right">
            ...
            <li class="breadcrumb-item"><a href="{{ action("Admin\BlacklistController@index") }}">{{ trans('messages.blacklist') }}</a></li>
        </ul>
        <h1>
...
@section('content')
    <div class="row">
        <div class="col-md-6">
            <form id="importForm" action="{{ action('Admin\BlacklistController@startImport') }}" method="POST" class="form-validate-jquery" enctype="multipart/form-data">
                {{ csrf_field() }}

                <h2 class="text-semibold mt-0">{{ trans('messages.blacklist.upload_list_from_file') }}</h2>

                <p>{!! trans('messages.blacklist.import.intro') !!}</p>
