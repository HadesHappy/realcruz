                    ...
                    <span class="template-image-box2">
                        ...
                        <div class="preview_control">
                            <div style="width: calc(80% - 0px);">
                                <div class="mb-2">
                                    <a class="btn btn-light d-block" href="#preview" onclick="popupwindow('{{ action('Admin\TemplateController@preview', $template->uid) }}', `{{ $template->name }}`, 800)"><span class="material-symbols-rounded">
                                        zoom_in
                        ...
                        overflow: hidden;" class="card-text">
                            @if (Auth::user()->admin->can('update', $template))
                                <a href="{{ action('Admin\TemplateController@categories', [
                                    'uid' => $template->uid,
                                ]) }}" class="template-categories text-secondary">
                            ...
                            @else
                                <span class="template-categories">{{ $template->categories()->count() ? $template->categories->map(function ($cat) {
                                    return $cat->name;
                                })->join(', ') : 'N/A' }}</span>
                            @endif
                        </p>
