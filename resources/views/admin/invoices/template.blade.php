...
@section('menu_title')
    <li class="d-flex align-items-center">
        <div class="d-inline-block d-flex mr-auto align-items-center ml-1 lvl-1">
            <h4 class="my-0 me-2 menu-title">{{ trans('messages.invoice_template') }}</h4>
...
@section('content')
    <form id="classic-builder-form" action="{{ action('Admin\InvoiceController@template') }}" method="POST" class="ajax_upload_form form-validate-jquery">
        {{ csrf_field() }}
        ...
        <div class="row mr-0 ml-0">
            <div class="col-md-9 pl-0 pb-0 pr-0 form-group-mb-0">
                ...
                @include('helpers.form_control', [
                    ...
                    'type' => 'textarea',
                    'name' => 'content',
                    'value' => Acelle\Model\Invoice::getTemplateContent(),
                    'rules' => ['content' => 'required']
                ])                
            </div>
            <div class="col-md-3 pr-0 pb-0 sidebar pr-4 pt-4 pl-4" style="overflow:auto;background:#f5f5f5">
				<div class="tags_list">
                    <h5 class="text-semibold text-teal mb-2">{{ trans('messages.available_tags') }}:</h5>
                    @foreach(Acelle\Model\Invoice::getTags() as $tag)
                        @if (!$tag["required"])
