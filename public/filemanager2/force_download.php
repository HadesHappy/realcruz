...
if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
    response(xtrans('forbidden') . AddErrorLocation(), 403)->send();
    exit;
}

if (!checkRelativePath($_POST['path']) || strpos($_POST['path'], '/') === 0) {
    response(xtrans('wrong path') . AddErrorLocation(), 400)->send();
    exit;
}
...
if (!check_extension($info['extension'], $config)) {
    response(xtrans('wrong extension') . AddErrorLocation(), 400)->send();
    exit;
}
...
if ($ftp) {
    ...
    if (!file_exists($path . $name)) {
        response(xtrans('File_Not_Found') . AddErrorLocation(), 404)->send();
        exit;
    }
